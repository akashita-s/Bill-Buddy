...
export default function LiveExpenses() {
  ...
  if (loading) {
    return <p className="text-sm text-gray-500 dark:text-gray-400">Loading expenses…</p>
  }
  ...
  if (rows.length === 0) {
    return (
      <p className="text-sm text-gray-500 dark:text-gray-400">
        No expenses yet —{' '}
        <Link href="/add-expense" className="text-blue-600 hover:underline dark:text-blue-400">
          add one
        </Link>
        .
      </p>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900">
        ...
        <p className="mt-1 text-2xl font-bold text-red-600 dark:text-red-400">{currency(total)}</p>
      </div>
